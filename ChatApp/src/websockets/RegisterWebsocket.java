...
public class RegisterWebsocket implements MessageListener{
    ...
    public void onMessage(String message, Session session){
        ...
        try {
			...
			Requests.makePostRequest("http://localhost:8080/UserApp/rest/users/register", jsonUser);
		...
		for (String un : sessions.keySet()) {
			...
			if (sessions.get(un).isOpen())
				if (sessions.get(un) != null && sessions.get(un).isOpen()) {
		    		boolean exists= Data.userExists(un);
		    		if (exists) {
